...
| [test-frontend.ps1](dev/test-frontend.ps1) | `npm ci` + `npm test` en `frontend/` (`-SkipInstall` omite `npm ci`) | Parcial — añade `lint`/`typecheck` vía [devsecops-ci.md](../docs/engineering/devsecops-ci.md) |
...
.\scripts\dev\test-e2e.ps1                       # E2E local (Docker); en CI solo manual en Actions
