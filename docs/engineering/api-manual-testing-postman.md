...
Usuarios de prueba y OIDC en SPA: [jwt-gateway-strategy.md](../security/jwt-gateway-strategy.md) §4. Realm importado: [infra/compose/init/keycloak/mtl-realm.json](../../infra/compose/init/keycloak/mtl-realm.json).
...
Variables sugeridas: `gateway` → `http://localhost:8080`; `access_token` → token vigente.

Cabeceras: `Authorization: Bearer …`; opcional `X-Correlation-Id` (UUID).
